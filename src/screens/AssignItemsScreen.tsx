...
export function AssignItemsScreen({
  ...
  function selectedPeopleText(item: ReceiptItem) {
    ...
    if (item.assignedTo.length === people.length) return 'Everyone';
  ...
  return (
    <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.peopleGrid}>
        {people.map((person) => {
          const isSelectedForItem = selectedItem?.assignedTo.includes(person.id);
          return (
            <Pressable
              key={person.id}
              onPress={() => onTogglePerson(person.id)}
              onLongPress={() => previewPerson(person)}
              style={[styles.personBubble, isSelectedForItem && { borderColor: person.color, backgroundColor: hexToRgba(person.color, 0.12) }]}
            >
      ...
      <View style={styles.selectedItemCard}>
        <View style={styles.itemPagerRow}>
          ...
          <View style={styles.selectedItemMain}>
            ...
            <View style={styles.selectedItemTextBlock}>
              ...
              <Text style={styles.selectedPrice}>{money(selectedItem?.price ?? 0)}</Text>
              <Text style={styles.selectedAssigned}>{selectedItem ? selectedPeopleText(selectedItem) : 'Unassigned'}</Text>
            </View>
          </View>
        ...
        <Text style={styles.selectedHint}>Tap a person to mark this item with their color. Tap several people if it was shared.</Text>
      ...
      <View style={styles.receiptCard}>
        ...
        {items.map((item, index) => (
          <Pressable
            ...
            style={[styles.receiptRow, selectedItemId === item.id && styles.receiptRowSelected]}
          >
            ...
            <View style={styles.receiptItemBlock}>
              <Text style={styles.receiptItemName}>{index + 1}. {item.name}</Text>
        ...
        <View style={styles.legendWrap}>
          {people.map((person) => (
            <View key={person.id} style={styles.legendItem}>
              ...
              <Text style={styles.legendName}>{person.name}</Text>
            </View>
          ))}
        </View>
      </View>
...
function renderColorMarkers(item: ReceiptItem, people: Person[]) {
  if (item.assignedTo.length === 0) return <View style={styles.emptyMarker} />;
...
const styles = StyleSheet.create({
  ...
  personBubble: { width: '30%', minHeight: 112, borderRadius: 24, backgroundColor: theme.color.card, alignItems: 'center', justifyContent: 'center', gap: 5, borderWidth: 1.5, borderColor: theme.color.line, ...theme.shadow },
